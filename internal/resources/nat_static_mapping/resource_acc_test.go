package nat_static_mapping_test

// Acceptance test for hyperv_nat_static_mapping. Pairs a NAT switch
// (hyperv_virtual_switch with switch_type=NAT) with a nat_static_mapping
...
// TestAcc_NatStaticMapping_basic exercises create + in-place update of
...
func TestAcc_NatStaticMapping_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		CheckDestroy: resource.ComposeAggregateTestCheckFunc(
			acctest.CheckResourceGone("hyperv_nat_static_mapping",
				func(ctx context.Context, _ string) (*hyperv.NatStaticMapping, error) {
					return client.GetNatStaticMapping(ctx, hyperv.GetNatStaticMappingInput{
						NatName:           natName,
						Protocol:          "tcp",
		...
		Steps: []resource.TestStep{
			{
				Config: netNatStaticMappingConfig(switchName, natName, 30080),
				ConfigStateChecks: []statecheck.StateCheck{
					statecheck.ExpectKnownValue("hyperv_nat_static_mapping.test",
				...
				ConfigPlanChecks: resource.ConfigPlanChecks{
					PreApply: []plancheck.PlanCheck{
						plancheck.ExpectResourceAction(
							"hyperv_nat_static_mapping.test",
			...
			},
			{
				...
				ImportStateIdFunc: func(s *terraform.State) (string, error) {
					rs, ok := s.RootModule().Resources["hyperv_nat_static_mapping.test"]
					if !ok {
						return "", fmt.Errorf("hyperv_nat_static_mapping.test not in state")
					}
...
// netNatStaticMappingConfig renders a NAT-switch resource paired with a
// nat_static_mapping that consumes its nat_name. The internal address
...
func netNatStaticMappingConfig(switchName, natName string, internalPort int) string {
	return fmt.Sprintf(`
...
resource "hyperv_nat_static_mapping" "test" {
