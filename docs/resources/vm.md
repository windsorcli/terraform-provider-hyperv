...
- `notes` (String) Free-form description stored on the VM by Hyper-V. **Empty-vs-omitted caveat:** the host represents "no notes" as an empty string; the provider collapses that to null in state so omitting `notes` from config is stable across plans. Setting `notes = ""` to explicitly clear would loop -- omit the attribute instead.
