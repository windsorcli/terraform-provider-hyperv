...
2. Runs GoReleaser in release mode. It cross-compiles 13 binaries (linux / darwin / windows / freebsd × supported arches per [.goreleaser.yml](../../.goreleaser.yml)), zips each, generates `SHA256SUMS`, GPG-signs the checksums file, and attaches the registry manifest.
3. Creates the GitHub Release as a **draft**. Drafts are mutable, so all 16 assets (13 zips + `SHA256SUMS` + signature + manifest) upload cleanly.
4. The final workflow step flips the draft to **published**. At that moment the release becomes immutable — no further asset changes possible. Every byte that ships was already in place.
5. The Terraform Registry's webhook fires on the publish event and pulls the manifest. The new version becomes installable via `required_providers` within a minute or two.
...
- **release-drafter's notes don't auto-flow into the published release body.** Release-drafter maintains a *tagless* draft; GoReleaser creates a separate *tagged* draft on tag push. The published release ends up with an empty body. For now, copy/paste from the orphaned tagless draft into the published release manually, or rely on `CHANGELOG.md` (which release-drafter does maintain). Wiring release-drafter's body through to the goreleaser publish step is a follow-up.
