...
- **Local Administrators** is required for: `hyperv_nat_static` (`Add-NetNatStaticMapping` and `New-NetFirewallRule` return "Access denied" for Hyper-V Administrators alone); and `hyperv_virtual_switch` with `switch_type = "NAT"` (the underlying `New-NetNat` returns the same). `switch_type = "External"` was not directly tested — Local Administrators is the recommended floor.
